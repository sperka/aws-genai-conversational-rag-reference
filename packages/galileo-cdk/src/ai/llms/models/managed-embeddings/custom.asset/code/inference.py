...
def predict_fn(input_object, config):
    ...
    if not current_model_config:
        raise ValueError(
            f"Model {current_model_id} not found: available models {model_ids}")

    current_model = current_model_config["model"]
    current_tokenizer = current_model_config["tokenizer"]
    current_is_cross_encoder = input_object.get(
        "type", get_model_type(current_model_id)) == TYPE_CROSS_ENCODER
